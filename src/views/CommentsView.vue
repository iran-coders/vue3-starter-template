...
<VForm class="row align-items-end">
    <div class="col-3">
        <VSelect
            ...
            item-key="key"
            item-text="text"
            :placeholder="$t('Response All')"
            clearable
        >
            <template #label>{{ $t("Response All") }}</template>
        </VSelect>


    </div>
    <div class="col-3">
        <button @click="handleResponseAllChange" class="btn btn-secondary">
            {{ $t('Confirm') }}
        </button>
    </div>
</VForm>
...
const {commentsStore, confirm, reject, showInfo, showInfoModal, relatedPost, commentModalTitleText, selectedItem, selectedResponse, responseItems, handleResponseAllChange} = useComments();
